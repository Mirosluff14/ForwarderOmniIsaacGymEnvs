...
class ForwarderPickTask(RLTask):
    ...
    def calculate_metrics(self) -> None:
        ...
        close_to_wood = (wood_grappler_reward > 0.8).float()

        reward = wood_grappler_reward
        # Add reward for grapple being closed (sides are close to each other)
        #reward = self.calculate_distance_reward(self.grapple_l_pos, self.grapple_r_pos) #* close_to_wood

        # Add a check if the log has not been delivered
        not_delivered = (torch.ones_like(self.reward)).float()

        # Add reward for lifting the wood
        reward *= 1 + self.wood_pos[:, 2] * close_to_wood * 10

        # Set reward for getting close to the target
        wood_target_reward = self.calculate_distance_reward(self.wood_pos, self.target_pos)

        # Multiplier for getting close to the target
        close_to_target = (wood_target_reward > 0.95).float() 

        # Add reward for getting close to the target while having the wood in the grapple
        reward *= 1 + wood_target_reward * close_to_target * close_to_wood  *  100 * not_delivered

        # might need to add the reward for dropping the wood and lifting the grapper when close to the target
        # it means that the grapple is high and far from the wood and the wood is close to the target
        not_delivered *= (1 - close_to_target)

        reward *= 1 + self.grapple_body_pos[:, 2] * (1 - close_to_wood) * close_to_target * 10000
        print('Reward: ', torch.mean(reward), 'Close to wood: ', torch.mean(close_to_wood), 'Close to target: ', torch.mean(close_to_target))

        # If the wood is closer than 10cm to the target, give a bonus of 100
        #reward += 100. * (torch.norm(to_target, p=2, dim=-1) < 0.1).float()
